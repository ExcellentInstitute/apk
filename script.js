...
// =========================================================
// UNIFIED DUE CALCULATION ENGINE (Exact Port from Dart)
// =========================================================
function calculateExactDues(student) {
    ...
    let totalOutstanding = totalFee - paidFee - adDiscount;
    ...
    if (feeStructure.toLowerCase().includes('one-time') || feeStructure.toLowerCase().includes('onetime') || feeStructure.toLowerCase().includes('lumpsum')) {
        ...
        if (advanceFee === 0) advanceFee = paidFee;
        if (advanceFee > paidFee) advanceFee = paidFee;
        ...
        try {
            ...
            monthsPassed = stTx.length > 0 ? stTx.length : 0;
        }
        ...
        currentMonthDue = Math.ceil(expectedPaidByNow - paidFee);
        
        if (currentMonthDue < 0) currentMonthDue = 0.0;
        else if (currentMonthDue > totalOutstanding) currentMonthDue = totalOutstanding;
...
function selectStudent(id) {
    ...
    if (waBtn && !document.getElementById('custom-notify-btn')) {
        ...
        notifyBtn.id = 'custom-notify-btn';
        notifyBtn.type = 'button';
        notifyBtn.className = 'w-8 h-8 rounded-full bg-amber-500/20 hover:bg-amber-500 text-amber-500 hover:text-white flex items-center justify-center transition-all ml-2';
        notifyBtn.title = 'Send App Notification';
        notifyBtn.innerHTML = '<i class="fa-solid fa-bell text-xs"></i>';
        notifyBtn.onclick = () => sendCustomPushNotification();
        waBtn.parentNode.insertBefore(notifyBtn, waBtn.nextSibling);
    }
